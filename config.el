...
(setq tatsuo/lisp-implementations
      '((sbcl ("sbcl"))))
...
;;; tabs
(def-package! awesome-tab
  :load-path "~/.doom.d/packages/awesome-tab"
  :hook (after-init . awesome-tab-mode))

(after! (helm awesome-tab)
...
(after! sly
  ...
  (sp-local-pair '(sly-mrepl-mode) "`" "`" :actions nil)
 ; (evil-define-key* 'insert sly-mrepl-mode-map ;;new
 ;   [S-return] #'newline-and-indent)
  (setq sly-lisp-implementations tatsuo/lisp-implementations
...
(defun tatsuo/smarter-move-beginning-of-line (arg)
  (interactive "^p")
  (setq arg (or arg 1))
  (when (/= arg 1)
   ...
   (process-buffer
    (if-let* ((pipenv (executable-find "pipenv"))
        ...
        (let ((default-directory pipenv-project)
          ...
          (run-python nil t t))
      (run-python nil t t)))))
(advice-add #'+python/repl :override #'+python*repl)
